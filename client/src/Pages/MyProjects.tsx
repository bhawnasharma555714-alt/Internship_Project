...
function MyProjects(){
    ...
    return(
        <Layout>
            ...
            <p className="mt-2 md:mt-4 text-xl text-slate-400 max-w-2xl mx-auto text-center">Browse your projects</p>
            ...
            {displayedProjects.length === 0 ? (
                <div className="text-center">
                    <h2 className="text-slate-400 p-4 text-2xl md:text-3xl">No projects found</h2>
                    <p className="text-slate-500 p-2 text-sm md:text-xl">Try searching with different keywords.</p>
                </div>
                ...
                <div key={project.id} className="max-w-3xl mx-auto border-4 border-slate-700 mt-10 p-10 text-left rounded-2xl hover:border-slate-600 hover:shadow-[0_0_20px_rgba(14,165,233,0.08)]">
                    <h2 className="text-2xl md:text-4xl font-bold text-white py-2">{project.title}</h2>
                    <div className="max-w-3xl leading-8 text-slate-300 pt-2">
                    ...
                    <div className="flex flex-wrap gap-3 mt-3">
                        {project.requiredSkills.map((skill,index) => (
                            <span key={`${skill}-${index}`} className="bg-sky-100 text-sky-900 py-1.5 px-3.5 md:px-5 md:py-2 rounded-full font-semibold transition-all duration-200 hover:-translate-y-1 hover:scale-110 hover:cursor-pointer">{skill}</span>
                        ))}
                    </div>
                    ...
                    <div className="mt-1 flex flex-col justify-between items-center md:flex-row lg:flex-row">
                        ...
                        <div className="flex items-center gap-3">
                            <button className="bg-green-700 text-white font-medium px-6 md:px-8 py-2 mt-6 rounded-lg hover:bg-green-600 transition-colors" onClick={()=>navigate(`/project/${project.id}/edit`)}>Edit Project</button>
